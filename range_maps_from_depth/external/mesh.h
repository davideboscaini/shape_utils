...
public:
...
#ifdef USE_FLANN
	mesh_t::mesh_t() : flann_index(0), flann_data(0), flann_data_mat(0) {}
	...
	size_t n_points() const { return vertices.size(); } //mesh_t:: 
	
	size_t n_faces() const { return triangles.size(); } //mesh_t::

   const oriented_point& get_vertex(int i) const { return vertices.at(i).p; }
   
#ifdef USE_FLANN
   void mesh_t::update_kd_tree()
   {
		if (flann_index)
	   {
			delete[] flann_data_mat->ptr();
   ...
   void put_vertices(const std::vector< vec3d<double> >& points) //mesh_t::
	{
	...
	void add_triangle(int p0, int p1, int p2) //mesh_t::
	{
	   triangle_data td;
	 ...
	 */
	void mesh_t::nearest_neighbors_with_dist(int p, double radius, std::vector<int>& neighs, std::vector<double>& dists)
	{
	   if (!flann_index)
			update_kd_tree();
	 ...
	 * @param neighs
	 */
	 void get_tri_imm_neighbors(int tri, std::vector<int>& neighs) const //mesh_t::
	 {
