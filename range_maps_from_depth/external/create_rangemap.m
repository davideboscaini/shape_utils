function [N, depth, matches] = create_rangemap(M, w, h)
...
% [w] comment out the following two lines to obtain the original code:
step_x = mean([(max_x - min_x) / (w-1),(max_y - min_y) / (h-1)]);
step_y = step_x;
...
% [w] comment out the following three lines to obtain the original code:
tmp             = N.VERT(:,3); 
tmp(isnan(tmp)) = 999;
N.VERT(:,3)     = tmp;
...
% t    = 4*res; % [w]
% keep = d1 < t & d2 < t & d3 < t; % [w]

%N.TRIV = N.TRIV(keep,:); % [w]
N.m    = size(N.TRIV,1);
