...
%full_model = load_off('../RandomForests/faust_templates/tr_reg_000.off');

tmp = load('G:\dboscaini\works\projects\learning_func_maps_data\datasets\FAUST_registrations\meshes\rototranslated\tr_reg_000.mat');
full_model.VERT = [tmp.shape.X,tmp.shape.Y,tmp.shape.Z];
full_model.TRIV = tmp.shape.TRIV;
...
for a=linspace(0,2*pi,10)
    ...
    [M, depth, matches] = create_rangemap(S, 100, 170);
