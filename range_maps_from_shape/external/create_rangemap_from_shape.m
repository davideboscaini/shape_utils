function [N, depth, matches] = create_rangemap_from_shape(M, w, h, params)

O      = struct;
O.VERT = [params.original_shape.X, params.original_shape.Y, params.original_shape.Z];
O.TRIV = params.original_shape.TRIV;
d1 = sqrt(sum((O.VERT(O.TRIV(:,1),:)-O.VERT(O.TRIV(:,2),:)).^2,2));
d2 = sqrt(sum((O.VERT(O.TRIV(:,1),:)-O.VERT(O.TRIV(:,3),:)).^2,2));
d3 = sqrt(sum((O.VERT(O.TRIV(:,3),:)-O.VERT(O.TRIV(:,2),:)).^2,2));
res = median([d1;d2;d3]);
